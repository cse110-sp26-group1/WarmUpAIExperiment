...
const SPIN_PREVIEW_INTERVAL = 90;

// Weighted symbol list keeps the slot machine outcomes intentionally uneven.
const symbols = [
...
let audioContext;
let reelPreviewTimers = [];

// Cache all frequently updated DOM nodes once so the spin loop stays lightweight.
...
function loadState() {
  // Fallback state is used for first load or if localStorage data becomes invalid.
  const fallback = {
...
function persistState() {
  // Store only serializable symbol ids so saved data stays compact and resilient.
...
function startReelPreview(index) {
  renderSymbol(index, getRandomSymbol());
  reelPreviewTimers[index] = window.setInterval(() => {
    renderSymbol(index, getRandomSymbol());
  }, SPIN_PREVIEW_INTERVAL);
}

function stopReelPreview(index) {
  if (reelPreviewTimers[index]) {
    window.clearInterval(reelPreviewTimers[index]);
    reelPreviewTimers[index] = null;
  }
}

function stopAllReelPreviews() {
  reels.forEach((_, index) => stopReelPreview(index));
}
...
function getRandomSymbol() {
  // Roll against the combined weight total to bias common joke symbols higher.
