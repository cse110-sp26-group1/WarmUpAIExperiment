/*
  ...
  updates the live feed, and renders the slot game history timeline.
  Notes: Existing gameplay remains intact while helper functions keep the new
  educational UI content consistent with the app's satire-heavy presentation.
*/
...
// Keep the history content data-driven so the new UI section stays easy to
// maintain without scattering timeline copy throughout the markup.
const slotHistory = [
  {
    year: "1890s",
    title: "Mechanical Origins",
    description:
      "Early slot machines used physical reels, springs, and simple symbols to turn chance into a compact arcade spectacle."
  },
  {
    year: "1900s-1960s",
    title: "Mass-Market Cabinets",
    description:
      "Electromechanical cabinets spread through bars and casinos, making slots louder, brighter, and easier to scale as a business."
  },
  {
    year: "1970s-1990s",
    title: "Video Slot Expansion",
    description:
      "Digital displays replaced pure mechanics, allowing bonus rounds, richer visuals, and more elaborate reward illusions."
  },
  {
    year: "2000s-Present",
    title: "Online Optimization Era",
    description:
      "Modern slots moved onto networked platforms where telemetry, retention tactics, and themed skins became core product features."
  },
  {
    year: "Today",
    title: "AI-Flavored Monetization",
    description:
      "Token Tilter 3000 satirizes the current trend: wrap familiar gambling loops in AI language, then call the spend curve innovation."
  }
];
...
function renderSlotHistory() {
  ...
  slotHistory.forEach((entry) => {
    const item = historyItemTemplate.content.firstElementChild.cloneNode(true);
    item.querySelector(".history-year").textContent = entry.year;
    item.querySelector(".history-name").textContent = entry.title;
    item.querySelector(".history-copy").textContent = entry.description;
